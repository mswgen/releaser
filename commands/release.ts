...
`
      ...
      client.channels.cache.get('844136138251173898').send(`${interaction.data.options.find(x => x.name == 'repo').value} 버전 ${newVer}이 릴리즈되었어요!`, new Discord.MessageAttachment(Buffer.from(changelogmd), 'CHANGELOG.md')).then(m => m.crosspost())
